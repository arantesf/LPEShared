...
namespace Revit.Common
{
    ...
    public class RestaurarPisosEJuntas : IExternalCommand
    {
                    ...
                    foreach (var stringAmbiente in checkedAmbientes)
                    {
                        ...
                        foreach (var pair in sameSlopeFloors)
                        {
                            ...
                            foreach (Floor newFloor in newFloors.Select(a => a.Value))
                            {
                                ...
                                foreach (Floor floor in pair.Value)
                                {
                                ...
                                tx.Start();
                                foreach (Floor floor1 in pair.Value)
                                {
                                    foreach (Solid item in floor1.get_Geometry(new Options()))
                                    {
...
#endif
                            dontReset:
                                Utils.CopyAllParameters(pair.Value[0], newFloor, new List<BuiltInParameter>() { BuiltInParameter.ALL_MODEL_MARK, BuiltInParameter.FLOOR_HEIGHTABOVELEVEL_PARAM });
